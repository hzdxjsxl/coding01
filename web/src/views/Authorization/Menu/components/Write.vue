...
import { getMenuListApi, submitMenuApi } from '@/api/menu'
...
const submit = async () => {
  ...
  // console.log('elForm?.validate()', elForm?.validate())
  // const valid = await submitMenuApi(elForm?.validate()).catch((err) => {
  //   console.log('err', err)
  // })
  // if (valid) {
  ...
  // }
  const elForm = await getElFormExpose()
  if (!(await elForm?.validate())) return
  const formData = await getFormData()
  console.log('formData', formData)
  // 把 meta 里的属性全铺平到最外层，然后删掉原有的 meta 对象，保持结构极简
  const submitData = {
    ...formData,
    ...formData.meta
  }
  delete submitData.meta
  await submitMenuApi(submitData)
  // 2. 直接拿你封装好的表单数据
  return formData
}
